    ...
    const url = api_url + "/" + todo.id; //どのデータを操作するか伝える
        ...
        fetch(url, {
            ...
            body: JSON.stringify({ //データをJSON形式に変換(オブジェクト→JSON)
                priority: !todo.priority === true //trueとfalseをひっくり返す = ★あり（true） → クリック → ★なし（false）/★なし（false） → クリック → ★あり（true）
...
const updateTitle = (todo) => {
    ...
    const newPriority = prompt("優先度を入力(high / medium / low)", todo.priority); //優先度を入力できるポップアップ
